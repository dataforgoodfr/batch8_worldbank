...
def update_charts(disaster, year, ImpactType):
    ...
    fig = px.histogram(
        ...
        nbins=bins,
        # barmode="stack",
        title="Evolution of {0} caused by {1} per Regions".format(
            impact_type, disaster
        ),
    ...
    fig3 = px.line(
        df_fig, x="Decade", y="Temperature", labels={"°C": "Average Temperature"}
    )
    fig3.update_traces(
    ...
    fig4 = px.histogram(
        df.query("Decade >=@year[0] and Decade <@year[1]"),
        x="Decade",
        y=impact_type,
        color="Disaster_Type",
